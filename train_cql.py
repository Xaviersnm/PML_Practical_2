...
def cql_eval(env, config, q_table, eval_episodes=500, output=True):
    """
    Evaluate CQL configuration (Modified for Single Joint Q-Table)
    """
    eval_agents = CQL(
    ...
    # Assign the single learned Q-table
...
def train(env, config, output=True):
    """
    Train and evaluate Central Q-learning
    """
    # Initialize CQL instead of IQL
    agents = CQL(
        num_agents=env.n_agents,
        action_spaces=env.action_space,
        gamma=config["gamma"],
    ...
    # This list will store copies of the single joint q_table
    evaluation_q_tables = []
    ...
    for eps_num in range(config["total_eps"]):
        ...
        if eps_num > 0 and eps_num % config["eval_freq"] == 0:
            # Pass the single table to the evaluation function
